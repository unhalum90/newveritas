...
export function StudylabListClient() {
    ...
    // Filter for studylab activities only
    const studylabAssessments = useMemo(() => {
        return assessments.filter((a) => a.type === "studylab");
    }, [assessments]);
    ...
    const { activeAssessments, pastAssessments } = useMemo(() => {
        ...
        return {
            activeAssessments: studylabAssessments.filter(isActive),
            pastAssessments: studylabAssessments.filter((a) => !isActive(a)),
        };
    }, [studylabAssessments]);
        ...
        <div className="min-h-screen bg-zinc-50 px-6 py-10">
            <div className="mx-auto w-full max-w-4xl space-y-6">
                ...
                <div className="flex items-center gap-2 text-sm text-zinc-500">
                    ...
                    <span>/</span>
                    <span className="text-zinc-900 font-medium">Study Lab</span>
                </div>

                {/* Page Header */}
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-semibold text-zinc-900">🧪 Study Lab</h1>
                        <p className="mt-1 text-sm text-zinc-600">Practice activities and study sessions</p>
                    </div>
                </div>
                ...
                {!loading && displayedAssessments.length === 0 && (
                    <div className="text-center py-12 text-zinc-500 bg-white rounded-lg border border-dashed border-zinc-200">
                        ...
                        <p>
                            {view === "active" ? "No active study lab activities right now." : "No past study lab activities yet."}
                        </p>
                    </div>
                )}
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        {displayedAssessments.map((a) => (
                            <Card key={a.id} className="overflow-hidden">
                                ...
                                <CardContent>
                                    <Link href={`/student/studylab/${a.id}`}>
