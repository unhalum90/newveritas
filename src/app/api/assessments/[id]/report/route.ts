...
export async function GET(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { data: assessment, error: assessmentError } = await supabase
    .from("assessments")
    .select("id, latest_report_id, scores_last_modified_at")
    .eq("id", assessmentId)
    .maybeSingle();
  ...
  if (!assessment.latest_report_id) {
    const res = NextResponse.json({ report: null });
  ...
  const { data: report, error: reportError } = await supabase
    ...
    .eq("id", assessment.latest_report_id)
    .maybeSingle();

  if (reportError) return NextResponse.json({ error: reportError.message }, { status: 500 });

  const scoresLastModified = assessment.scores_last_modified_at
    ? new Date(assessment.scores_last_modified_at).getTime()
    : null;
  const reportGenerated = report?.generated_at ? new Date(report.generated_at).getTime() : null;
  const stale = scoresLastModified != null && reportGenerated != null ? scoresLastModified > reportGenerated : false;

  const res = NextResponse.json({ report: report ?? null, stale });
  pendingCookies.forEach(({ name, value, options }) => res.cookies.set(name, value, options));
  return res;
}
